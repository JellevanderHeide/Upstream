    ...
    protected void spawnEntities() {
        ...
        switch (hazardType) {
            ...
            case FISHHOOK:
                ...
                break;
            case FISHNET:
                spawn(new FishNet(new Coordinate2D(sceneWidth, random.nextDouble(0, sceneHeight / upstream.getTileHeight() * upstream.getTileHeight() - 2 - FishNet.getSpriteHeight())), 4));         
                break;
            case RAPIDS:
                spawn(new Rapids(new Coordinate2D(sceneWidth, random.nextDouble(0, sceneHeight / upstream.getTileHeight() * upstream.getTileHeight() - 2 - Rapids.getSpriteHeight())), 4));       
            case RIVERTRASH:
                spawn(new RiverTrash(new Coordinate2D(random.nextDouble(0, sceneWidth- RiverTrash.getSpriteWidth()), 0), 2));
                break;
            case ROCK:
